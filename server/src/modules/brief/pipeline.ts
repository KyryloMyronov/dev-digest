 ...
 * at `error`.
 *
 * THAT LOG REACHES THE SERVER CONSOLE AND NOTHING ELSE. An earlier version of
 * this comment claimed `error` was "the only level that reaches the user as a
 * toast" — it is not, for this module. `RunLogger.event` fans out with
 * `for (const runId of this.runIds)` (`platform/run-logger.ts:63`) and
 * `BriefService.runBriefDerivation` constructs the logger with `[]`, which it
 * says so itself ("the events mirror to pino only", `service.ts`). So a failed
 * derivation is, by construction, indistinguishable from an idle one at the
 * studio: no row (AC-11), no rejected job (AC-10), and a `role="status"` region
 * that is `sr-only`. That combination is how a dead `Derive brief` button went
 * unnoticed (2026-08-29, `server/insights.md`). Do not lean on this log to
 * inform a user — if a failure must be user-visible, give the derivation a run
 * stream or surface the outcome through the card.
 *
