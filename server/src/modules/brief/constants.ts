 ...
 *
 * 8_000, NOT 2_000 (2026-08-29): a REASONING model spends reasoning tokens out
 * of this same budget before it emits a single character of JSON. On
 * `deepseek/deepseek-v4-flash` — the model `review_intent` already defaults to,
 * and the obvious pick once OpenAI is out of credits — 2_000 gave
 * `finish_reason=length` with `completion_tokens=2000` on 4 of 4 calls, three
 * of them returning EMPTY content, so `parse_failed`/`llm_failed` every time
 * while the same prompt needs only ~1_700 tokens of actual answer. Check
 * `reasoning` / `reasoning_effort` in a model's OpenRouter
 * `supported_parameters` before lowering this again.
 */
export const BRIEF_MAX_OUTPUT_TOKENS = 8_000;
