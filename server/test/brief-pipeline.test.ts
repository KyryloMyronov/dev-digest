 ...
 *
 * IT HONOURS `max_tokens` (2026-08-29). A real provider stops generating at the
 * cap and hands back a truncated body, which then fails the schema; the version
 * of this stub that ignored `req.maxTokens` made an undersized
 * `BRIEF_MAX_OUTPUT_TOKENS` unobservable in CI, and it shipped that way — see
 * `server/insights.md`. Model a cap-ignoring provider explicitly with
 * `ignoresMaxTokens`, which is AC-16's defence-in-depth case and nothing else.
 */
class StubLlm implements Partial<LLMProvider> {
  ...
  constructor(
    private result: {
      ...
      costUsd?: number | null;
      /**
       * Behave like a provider that OVERRUNS `max_tokens` instead of stopping
       * at it. Only AC-16 wants this: clamping exists precisely for a provider
       * that hands back more than it was asked for.
       */
  ...
  async completeStructured<T>(req: StructuredRequest<T>): Promise<StructuredResult<T>> {
    ...
    const raw = JSON.stringify(this.result.data);
    // `approxTokens`, not the real encoder: a stub only has to be monotonic in
    // the answer's size to catch a grossly undersized cap, and chars/4 keeps
    // this hermetic and instant. The exact-encoder check is the headroom test.
    if (!this.result.ignoresMaxTokens && req.maxTokens !== undefined
        && approxTokens(raw) > req.maxTokens) {
      // What the real providers do with a truncated body — `parseWithRepair`
      // fails and they throw (`reviewer-core/src/llm/openrouter.ts:128`).
      throw new Error(
        `structured output failed schema validation for ${req.schemaName} ` +
          `(stopped at max_tokens=${req.maxTokens})`,
      );
    }
    return {
...
describe('the model call', () => {
  it('AC-14 issues exactly ONE structured request, with maxRetries 0', async () => {
    ...
    // NFR-4's output half is set explicitly, never omitted. Asserted against
    // the constant, not a literal: the VALUE is a model-dependent tuning knob
    // (a reasoning model spends this budget before emitting JSON), while
    // "explicitly set" is the invariant this test is actually for.
    expect(llm.calls[0]!.maxTokens).toBe(BRIEF_MAX_OUTPUT_TOKENS);
    expect(llm.calls[0]!.maxTokens).toBeGreaterThan(0);
  });
...
describe('AC-16 — clamps applied in code, because strict json_schema ignores .max()', () => {
  it('clamps every array and every string to its declared limit', async () => {
    ...
    const { repo } = makeRepoStub();
    // `ignoresMaxTokens`: this fixture is deliberately far larger than any cap,
    // because the clamp is defence-in-depth against a provider that overruns
    // `max_tokens`. A cap-honouring stub would truncate it and never reach the clamp.
    const { container } = makeContainer({
      llm: new StubLlm({ data: oversized, ignoresMaxTokens: true }),
    });
...
// --------------------------------------------- the output-token budget (2026-08-29)

/**
 * The largest answer the schema and `clampBrief` between them permit: every
 * array at its cap, every string at its ceiling. Anything the model can
 * legitimately return is no bigger than this.
 */
function maximalAnswer() {
  // A realistic monorepo path, not `a.ts` — path length is repeated 25× here
  // and is a real part of the payload.
  const file = 'server/src/modules/some/deeply/nested/path/file-name.ts';
  return {
    why_summary: 'w'.repeat(MAX_WHY_SUMMARY_CHARS),
    why_sources: Array.from({ length: 9 }, () => `diff:${file}`),
    risks: Array.from({ length: MAX_BRIEF_RISKS }, () => ({
      kind: 'error_handling',
      title: 't'.repeat(MAX_RISK_TITLE_CHARS),
      explanation: 'e'.repeat(MAX_RISK_EXPLANATION_CHARS),
      severity: 'CRITICAL',
      file,
      start_line: 1234,
      end_line: 1240,
    })),
    focus: Array.from({ length: MAX_FOCUS_ENTRIES }, () => ({
      file,
      start_line: 12,
      end_line: 34,
      reason: 'r'.repeat(MAX_FOCUS_REASON_CHARS),
    })),
  };
}

/**
 * An answer that overruns the shipped cap, so the provider stops mid-JSON.
 * Sized off the constant rather than hard-coded, so it stays over-cap whatever
 * the constant becomes.
 */
function overCapAnswer() {
  const base = maximalAnswer();
  const risks = [...base.risks];
  while (approxTokens(JSON.stringify({ ...base, risks })) <= BRIEF_MAX_OUTPUT_TOKENS) {
...
describe('output token budget', () => {
  /**
   * THE GUARD THIS MODULE SHIPPED WITHOUT.
   *
   * `BRIEF_MAX_OUTPUT_TOKENS` was originally 2_000, a figure derived backwards
   * from NFR-4's $0.07 cost ceiling (SPEC-02 — "a 2 000-token cap on output at
   * $8.00/1M = $0.016") rather than from how long an answer actually is. The
   * largest permitted answer measures ~6_250 tokens, so the cap could not hold
   * a full-size answer from ANY model, and every derivation that tried came
   * back truncated. No test could see it: the stub ignored `max_tokens` and
   * AC-14 asserted only `toBe(2_000)`, restating the constant.
   *
   * A cost ceiling is not a correctness bound. This asserts the correctness
   * bound directly, with the real encoder.
   */
  it('BRIEF_MAX_OUTPUT_TOKENS can hold the largest answer the schema and clamps permit', () => {
    // Pretty-printed: the conservative reading, and what models tend to emit.
    const json = JSON.stringify(maximalAnswer(), null, 2);
    const tokens = new TiktokenTokenizer().count(json);

    expect(tokens).toBeLessThanOrEqual(BRIEF_MAX_OUTPUT_TOKENS);
  });

  /**
   * The production failure, reproduced: `deepseek/deepseek-v4-flash` returned
   * `finish_reason=length` with `completion_tokens` exactly at the cap on 4 of
   * 4 calls — three of them with EMPTY content — so the derivation abandoned
   * every time and the studio showed a dead button. Reasoning tokens come out
   * of the same budget, which is why a cap sized to the answer alone is not
   * enough for a reasoning model.
   */
  it('abandons the derivation and persists NOTHING when the answer overruns the cap', async () => {
    const { repo, writes } = makeRepoStub();
    const { container } = makeContainer({ llm: new StubLlm({ data: overCapAnswer() }) });
    ...
    expect(out.reason).toBe('llm_failed');
    expect(writes).toHaveLength(0);
    // NFR-6: the abandonment is logged once, at `error`.
    expect(log.stdout.filter((l) => l.level === 'error')).toHaveLength(1);
  });
});
