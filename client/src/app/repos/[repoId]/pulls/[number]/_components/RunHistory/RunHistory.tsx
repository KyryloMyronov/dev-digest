        ...
        return (
          <div key={`run:${r.run_id}`} style={rowStyle}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 2, fontSize: 11, color: "var(--text-muted)", flexShrink: 0 }}>
              ...
              {/* Settled runs only — a null cost here means "model not priced",
                  never "hasn't finished", so it can read n/a unambiguously. */}
              {settled && (
                <span className="tnum">{formatCost(r.cost_usd, t("timeline.costUnknown"))}</span>
              )}
            </div>
            <button
              type="button"
