...
- **AC-21** — IF the token cap omitted any selected file, THEN the API shall
  record the omitted paths in that derivation's own outcome.
...
**Two different artifacts, deliberately not flattened.** **AC-21**'s omitted
paths are the **pipeline's own outcome** — the value the derivation returns to
its caller and names in its log line (**NFR-11**) — and that is a record of what
*this* derivation's token cap dropped. The wire field `omitted_files` on
`PrFileSummariesResponse` is a **different artifact with a different meaning**:
it is computed **at read time** over the derivation-eligible set (non-boilerplate
per **AC-18**, patch non-`null` per **AC-22**) and lists the eligible paths that
have **no summary at the PR's current head** — "eligible but not summarised",
whatever the reason. The table has no column recording a cap omission, so a
consumer of the response cannot tell "never derived" from "cap-omitted"; on
screen **AC-51**'s per-file control is what draws that distinction, because it is
offered on exactly the files with no stored summary. **AC-60** renders the
read-time counts, not AC-21's outcome.
...
- **AC-33** — WHEN a derivation succeeds, the API shall persist, per summarised
  ...
  timestamp, the provider, the model, and **that file's share of** the token
  counts and the cost.
...
**On AC-33's "share", and on what AC-34 no longer says.** A PR-level derivation
is **one** structured call over N files (**AC-24**), so the provider returns
**one** aggregate token count and **one** aggregate cost. Those aggregates are
apportioned across the derivation's rows in proportion to each file's prompt
tokens, remainder to the largest weight, so the sum across one derivation's rows
is **exact**: a `null` total makes every share `null`, a `0` total makes every
share `0`, and the two are never coalesced. That exactness is what makes
**NFR-1**'s ceiling and **AC-59**'s running total the same arithmetic, and it is
why a single row's `cost_usd` must never be read as the price of a call.

AC-34 previously also named a cached file as a `null`-cost case. **That clause
was vacuous and is removed**: under **AC-16** a file already stored at the
current head is skipped with no model request *and no write*, so no row is ever
persisted carrying `null` for that reason. The only live `null` case is an
unpriced model.
...
- **AC-60** — IF the file-summaries response reports fewer summarised files than
  eligible files, THEN the studio shall render a line stating how many of how
  many files were summarised.
...
| D-21 | a11y | **The file card header is a `<div onClick>`** — `FileCard.tsx:127`, no `role`, no `tabIndex`, no `aria-expanded`. The tab is not keyboard-operable today, and the mockup makes chevrons load-bearing | **A nested disclosure button.** The header row stays a non-interactive `<div>`; chevron + path + stat become one real `<button aria-expanded>`; the tag, the severity badge, the derive control and the comment count are its **siblings**, outside it. **The group header one file away (`SmartDiffGroups.tsx:59-64`) cannot be copied**: it has no interactive descendants, whereas this header already contains one `<button>` (the finding-jump badge) and **AC-51** adds a second, and a `<button>` may not contain interactive descendants. **Accepted cost, author-approved: clicking the badge strip no longer folds the card.** **WCAG 2.2 · 2.1.1 Keyboard (A)** and **4.1.2 Name, Role, Value (A)** | **AC-69**, **AC-70**, **P-3** |
...
- **`PrFileSummary` — NEW.**
  ...
  `insights.md:425-454` records for `PrMeta`. **`tokens_in`, `tokens_out` and
  `cost_usd` carry that file's apportioned *share* of one call's aggregate**, per
  **AC-33**, so `SUM(cost_usd)` over one derivation's rows is exact and a single
  row's value is never the price of a call. **`cost_usd` is `.nullable()`, not
  `.optional()`** — `null` is a *fact the studio must receive and render as a
  placeholder* ("the model is unpriced"), and an `.optional()`
...
- **`PrFileSummariesResponse` — NEW.** `{ summaries: PrFileSummary[], omitted_files: string[], selected: int, total: int }`.
  `omitted_files`, `selected` and `total` are **computed at read time** over the
  derivation-eligible set (non-boilerplate, patch non-`null`) and are what
  **AC-60** renders. `omitted_files` means *eligible but not summarised at the
  PR's current head* — it is **not** a record of what a token cap dropped; that
  is **AC-21**, which lives in the pipeline's own outcome and its log line, and
...
- **`head_sha` is part of the primary key, and therefore `NOT NULL`.** Unlike
  ...
  overwriting the description of the old patch. **The consequence, which NFR-5
  depends on:** rows accumulate per head SHA, so the read projects to
  **one row per path** — the current-head row when there is one, else the most
  recent stale row. Without that projection **AC-49** and **AC-58** would both
  fire for the same file and the response would grow without bound across
  force-pushes.
...
| **P-3** | Give the file card header a real control with an expanded state — **as a nested disclosure button**, not by making the header row itself a `<button>`; see D-21 | **accepted** → **AC-69**, **AC-70** | ~10 lines plus keyboard assertions. Closes a **shipped** WCAG 2.2 · 2.1.1 failure. Costs the badge strip its fold-on-click, because the header already owns interactive descendants. Collides with SPEC-02's edit to the same element — see *Sequencing* |
...
- **NFR-1 — Cost.** A PR-level derivation shall cost at most **$0.02**, measured
  ...
  one derivation. That sum is **exact**, not approximate: one call's aggregate is
  apportioned across the derivation's rows with the remainder assigned to the
  largest weight, so no rounding leaks out of the total (**AC-33**, **AC-59**).
  Derivation of the figure, at the price re-checked **2026-08-28** (`$0.08861` in
  / `$0.1772` out per 1M tokens for `deepseek/deepseek-v4-flash`): a derivation
  at the full **NFR-3** cap costs 48 000 × $0.08861/1M ≈ $0.0043 in, plus ~700
  output tokens ≈ $0.0001 out — **≈ $0.0044**, so the ceiling carries roughly 4×
  headroom for prompt overhead and price movement. **The price was re-checked and
  confirmed exactly on 2026-08-28** against `openrouter.ai/api/v1/models`:
  `prompt 0.000000088606` / `completion 0.000000177212` per token — **$0.088606
  in / $0.177212 out per 1M**, which matches the figures above to five
  significant figures. The repo's own table was **corrected in this build** from
  its stale `{ in: 0.077, out: 0.154 }`, which was **15.1% low**
  (`server/src/adapters/llm/pricing.ts:43` — the entry is that one line; the
  lines above it are its comment block, and the surrounding *"APPROXIMATE and
  must be confirmed against openrouter.ai/models"* warning still stands for every
  other OpenRouter row in the table). **OQ-3 is therefore closed.** Note that no
  figure here survives a workspace overriding `file_summary` to a different
  slug: the neighbouring `deepseek` slugs range from $0.03/$0.10 to $0.22/$0.66
  per 1M. Behaviour past the ceiling is **AC-20** and **AC-21** — the token cap
  is what makes the dollar figure true, and admitting fewer files is what happens
  when a PR is larger than the budget.
...
- **NFR-5 — Response size.** `GET /pulls/:id/file-summaries` shall return at most
  **32 KB** for a PR at the NFR-3 cap. Read off the serialised payload in an
  ...
  enforced at runtime — the row count is bounded by **AC-20**/**AC-21**, each
  row's length by **AC-74**, and, **critically, the read projects to one row per
  path**: the current-head row when there is one, else the most recent stale one.
  **That projection is the load-bearing half of the claim.** The table's primary
  key includes `head_sha`, so rows accumulate per head SHA; served unprojected,
  the payload would grow without bound across force-pushes and the other two
  bounds would not contain it. Measured on the build: **13 333 bytes for 28 rows
  — 41% of the ceiling — with 84 rows (28 paths × 3 head SHAs) in the table
  behind them.** So it has no separate past-it behaviour, and the integration row
  exists to catch the bounds drifting apart or the projection being lost.
...
Suites are named as [`TESTING.md`](../TESTING.md) names them. **One row per AC
and per NFR — 85 requirement rows, no requirement appearing twice and none
missing.** The table closes with **one further row that is not a requirement**,
marked `— (cost)` in the `Req` column: the shipped assertions this feature
changes. It is in the table rather than in a footnote because that cost is
invisible in every other row, and a reader counting requirement rows should
subtract it.
...
| AC-21 | `server-unit` | the `omitted` list on the pipeline's returned outcome for a fixture past the cap — **not** the response's `omitted_files`, which is a read-time computation |
...
| AC-34 | `server-integration` | `cost_usd` for an unpriced model and for a zero-priced model; and, for a file already stored at the current head, that **no row is written at all** (AC-16), which is why the removed cache clause was vacuous |
...
| AC-60 | `client` | the rendered "n of m" line for a payload whose `selected` is below its `total` |
...
| NFR-5 | `server-integration` | the byte length of the serialised response at the cap, with **more head SHAs in the table than paths**, so the row also proves the one-row-per-path projection is still in place |
...
| **— (cost)** | `client` | **the shipped `DiffTab.test.tsx` assertions this feature changes.** Not a requirement row — the cost of the rows above, made explicit because no other row shows it. **Ten** existing assertions are re-pointed, never deleted: **seven** predicted — `aria-expanded` counts and `getByRole("button", {expanded})` lookups broken by **AC-69** (a repeated component gaining `aria-expanded` poisons every such query in the suite), `"Core"` → `"Core logic"` by **AC-44**, `"Smart Diff · grouped by role"` by **AC-40**/**AC-41**, and `/Standard/` → `/Original order/` by **AC-42** — and **three** found only during the build, all `Found multiple elements with the text: "Critical"`, because **AC-65** puts the severity label at the line as well as in the header badge. Observation point: each re-pointed assertion, marked `SPEC-03 RE-POINTED` in the file with what it used to read. **SPEC-02's two reveal-focus assertions must pass unchanged** — that is the proof the two builds' `FileCard` edits compose |

## Open questions

Seven were raised. After the build: **two are closed outright** (OQ-3, OQ-6),
**two are closed in part** (OQ-1, OQ-2 — the provider-side half of each is
settled and moot; the residual half is named), and **three remain explicitly
deferred** (OQ-4, OQ-5, OQ-7). Each carries the assumption baked into this spec
and who decides. None blocks an implementer: every one names a value the spec
already carries and a condition under which that value would change. That is what
lets this spec sit at `approved`.

- **OQ-1 — Is 48 000 the right prompt token cap?** *Assumption baked in:* it is,
  and NFR-3 shows the arithmetic. **The provider half is now closed and moot.**
  The resolved model's context window is **1 048 576** tokens with max completion
  **384 000**, so 48 000 in plus the output allowance — 8 192 at the adapter's
  built-in default, 2 000 as `FILE_SUMMARY_MAX_OUTPUT_TOKENS` ships — is at most
  **~5% of the window**, and our cap fires roughly **20× below** the provider
  limit: `FILE_SUMMARY_PROMPT_TOKEN_CAP` is a
  **cost control, not a guard against provider rejection or silent truncation**,
  and neither of those outcomes is reachable by this feature. *What stays open is
  the content half:* the 1 698-tokens-per-file average in NFR-3 was extrapolated
  from four commits rather than measured on a real large diff. The build's first
  measurement of a generated fixture gave **~2 650 tokens per file and ~22 tokens
  per line** against NFR-3's 12.72 — real data, but from **synthetic** patch text,
  which runs longer than real diffs, so it is **not** a refutation. **The author
  ruled: record it, do not re-tune the cap.** The mechanism (**AC-20**) is
  independent of the number, so changing it stays a one-constant edit. *Decides:*
  the author, on the first real derivation of a large PR. **Deferred, in part.**
- **OQ-2 — How does OpenRouter aggregate structured-output support for an
  ...
  **model-id-level aggregate**, not the per-provider breakdown
  (`server/src/platform/model-catalog.ts:56-63`). **The default-configuration
  half is closed:** the bare slug `deepseek/deepseek-v4-flash` **does** advertise
  `structured_outputs` (and `response_format`) in its model-id-level
  `supported_parameters`, so **AC-29**'s preflight resolves `true` and the
  derivation proceeds in the shipped default configuration. *What stays open:*
  the per-provider variance behind the alias is **invisible to `ModelCatalog`**,
  which reads only that aggregate. *Consequence if it bites:* a routed request
  silently returns unstructured text, which **AC-27** catches as a parse failure
  — degraded, not incorrect, and **AC-27 is the standing catch for it**.
  *Decides:* the author, if parse failures cluster. **Deferred, in part.**
- **OQ-3 — ~~Is NFR-1's $0.02 still right at ship time?~~ Answered.** The price
  was re-checked and **confirmed exactly** on **2026-08-28** against
  `openrouter.ai/api/v1/models`: `$0.088606` in / `$0.177212` out per 1M, which
  matches the `$0.08861` / `$0.1772` NFR-1 records to five significant figures.
  `pricing.ts` was **corrected in this build** from its stale
  `{ in: 0.077, out: 0.154 }` — 15.1% low — so the repo's table and NFR-1's
  arithmetic now agree, and the $0.02 ceiling keeps its ~4× headroom. The
  standing caveat is not a question but a property, and it lives in NFR-1: no
  figure survives a workspace overriding `file_summary` to a different slug.
  **Closed.**
